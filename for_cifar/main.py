...
def main():
    ...
    ########### # List to store losses for visualize
    train_losses = []  
    test_losses = []   
    #########################
    ...
    if use_gpu:
        ...
        ############### use CosineAnnealingLR => if not use, remove this code  
    ...
    if not os.path.exists('result'):
        os.makedirs('result')
    fdir = 'result5/'+str(args.arch)+'_'+str(args.bit)+'bit'        ######################  result save code 
    ...
    normalize = transforms.Normalize(mean=[0.491, 0.482, 0.447], std=[0.247, 0.243, 0.262])            ########## for CIFAR 10 dataset
    #normalize = transforms.Normalize(mean=[0.5071, 0.4867, 0.4408],std=[0.2675, 0.2565, 0.2761])      ########## for CIFAR 100 dataset
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ###########################################
        adjust_learning_rate(optimizer, epoch)      #  to use CosineAnnealingLR, remove this code 
        ###########################################
        ...
        #################   
        #scheduler.step()           #  to use CosineAnnealingLR, remove this code 
        #################
    ...
    plt.plot(test_losses, label='Test Accuray')
    plt.title('Training and Test Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()       
    plt.savefig('resnet20_4bit_accuracy_on_cifar10.png')  # Save the plot as a PNG file             ####################################################
    plt.close()
